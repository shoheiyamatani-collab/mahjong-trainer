...
export const learnArticles: LearnArticle[] = [
  ...
  },
  {
    ...
    step: 11,
    title: "点数計算について",
    seoTitle: "麻雀の点数計算を初心者向けに解説 | 点数表の見方",
    description: "麻雀の点数が決まる仕組みと、親子・ロンツモ・符・翻の順に点数表を見る方法を初心者向けに説明します。",
    learnPoints: ["点数は符と翻の組み合わせで決まる", "親子とロン・ツモで支払いが変わる", "早見表を使って点数を確認する"],
    body: [
      "麻雀の点数は、役とドラの合計である翻と、手牌の形や待ち方から決まる符を組み合わせて計算します。",
      "点数表を見るときは、最初に自分が親か子か、次にロンかツモかを確認します。そのあと符と翻が交わる欄を見れば、支払う点数が分かります。",
      "すべてを暗記する必要はありません。よく出る30符・40符や満貫から見慣れ、迷ったときは点数早見表で確認しましょう。"
    ],
    example: "子のロンで30符2翻なら2000点です。同じ符と翻でも、親のロンなら2900点になるため、まず親か子かを確認します。",
    misconceptions: ["点数表をすべて暗記しないといけない", "ロンとツモで受け取る点数は同じ", "親と子で点数は変わらない"],
    quiz: {
      question: "点数表を見るとき、最初に確認したいことはどれですか？",
      choices: ["親か子か", "ドラ表示牌の位置", "残りの山の枚数"],
      answer: "親か子か",
      explanation: "親と子では同じ符・翻でも点数が変わります。続いてロンかツモかを確認します。"
    },
    relatedPractice: { label: "実践でよく見る点数計算を読む", href: "/rules/practical-score" },
    relatedTool: { label: "点数早見表を見る", href: "/tools/score-table" }
  }
];
